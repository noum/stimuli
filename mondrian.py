...
#
# created
# last mod 2012-10-30 11:19 KS

"""
This module offers functions to generate mondrian stimuli for the eizoGS320.
...
def distribute_patches(size, mean_edge_len, colors, weights=None, write=False, seed=1):
    """
    creates a mondrian display.
    ...
    mean_edge_len : int
        the mean length of edges of new patches. Actual edge length will
        randomly vary between 2/3 and 4/3 times this value, to allow for aspect
        ratios up to 2:1.
    colors : list of ints
        a list of possible gray values for the patches
    weights : list of floats (optional)
        a list of probabilities for the different colors. If not specified, all
        colors get equal probability.
    ...
    rng = WeightedRandomGenerator(weights, seed)
    M = -np.ones(size)
    x = 0
    if write:
...
def create_mondrian(size, mean_edge_len, colors, weights=None, accuracy=.05, max_cycles=1000, write=False, seed=1):
    """
    ...
    accuracy : float (optional)
        the maximally allowed deviation between the relative frequency of a
        color and its specified weight. Default is 0.05.
    max_cycles : int (optional)
        the maximal number of mondrians created before the function aborts and
        returns the best mondrian so far. Default is 1000.
    write : boolean (optional)
        if True, specification of how the Mondrian was constructed will be
    ...
    while True:
        counter += 1
        m = distribute_patches(size, mean_edge_len, colors, weights, write, seed)
...
def rotate_3d(m, rot_angle, ppd, constant_side='l', background=0, factor=1):
    ...
    rot_angle : number in ]90, 180[
        the angle between the resulting matrix and the viewing axis. 90 leaves
        the matrix unchanged, smaller values rotate it towards the observer,
        larger values away from the observer.
    ppd : number
        pixels per degree of visual angle of the target display.
    constant_side : ['l', 'r'], optional
        the side of the array used as the rotation axis. Default is 'l'.
    background : number, optional
        the value used to fill in the returned image in the locations not
        covered by the rotated surface. Default is 0.
    factor : number, optional
        if the function is called with a resized array, the resize factor can
        be passed to ensure that the output size is a multiple of the resize
        factor. Default is 1.
...
def overlapping_patch(left, right, size, value, y_offset=None, x_offset=None):
    ...
    y_offset : number, optional
        the vertical offset of the patch from the top of the image. By default,
        the patch will be placed in the center.
    x_offset : number, optional
        the horizontal offset of the patch from the midline. Should be a
        negative value such that the patch starts in the left half. Default is
        half of the horizontal patch size.
